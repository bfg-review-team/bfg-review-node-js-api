...
class MessageTrancactions extends FadabHelper{
    ...
    getMessage(values) {
        ...
        return queryAsync("SELECT * FROM Messages WHERE Id=? ",body_data)
    }
    ...
    addMessage(values) {
        ...
        return queryAsync("INSERT INTO Messages (UserId,MessageText,DiscussionId,CreatedDate) VALUES (?,?,?,?) ",body_data)   
    }
    deleteMessage(values) {
        const body_data = [values.Id]
        console.log('Deleting This Message !')
        return queryAsync("DELETE FROM Messages WHERE Id = ? ",body_data)
    }
    updateMessage(values) {
        ...
        return queryAsync("UPDATE Messages SET UserId = ?, MessageText = ?, DiscussionId = ?,CreatedDate = ? WHERE Id = ?",body_data)
    }
}

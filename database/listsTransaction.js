...
class ListTrancactions extends FadabHelper{
    ...
    getList(values) {
        ...
        return queryAsync("SELECT * FROM Lists WHERE Id=? ",body_data)
    }
    getUserLists(values) {
    ...
    addList(values) {
        ...
        return queryAsync("INSERT INTO Lists (ListType,UserId,MovieId) VALUES (?,?,?) ",body_data)   
    }
    deleteList(values) {
        const body_data = [values.Id]
        console.log('Deleting This List !')
        return queryAsync("DELETE FROM Lists WHERE Id = ? ",body_data)
    }
    updateList(values) {
        const body_data = [
            values.ListType,
        ...
        return queryAsync("UPDATE Lists SET ListType = ?, UserId = ?, MovieId = ? WHERE Id = ?",body_data)
    }
}
